<?php
/**
 * Williams Meerkat Info Widget
 * 
 * Display list of things.
 * Compact, attractive, noticeable
 * Click thing to reveal more info (top thing is active by default)
 * Reasonable simple edit interface
 * 
 * @package Meerkat_Info_Widget
 * 
 * @uses    plugin    Advanced Custom Fields
 * 
 * @author  Williams Web Team
 * @version 1.0.0
 */
/*
Plugin Name: Williams Meerkat Info Widget
Description: Display list of things. Compact, attractive, noticeable. Click thing to reveal more info (top thing is active by default). Reasonable simple edit interface.
Version: 1.0.0
Author: Williams Web Team
License: GPL3

Copyright 2013 Williams Web Team  (email : webteam@williams.edu)
...
class Meerkat_Info {
    var $namespace = "meerkat-info";
    var $friendly_name = "Meerkat Info Widget";
    var $version = "1.0.0";
     ...
     * 
     * @uses add_action()
     * @uses Meerkat_Info::wp_register_scripts()
     * @uses Meerkat_Info::wp_register_styles()
     */
    function __construct() {
        ...
        $libs = glob( MEERKATINFO_DIRNAME . '/lib/*.php' );
        foreach( $libs as $lib ) {
            include_once( $lib );
        }
         ...
         */
        load_theme_textdomain( $this->namespace, MEERKATINFO_DIRNAME . '/languages' );
    ...
    function admin_options_page() {
        ...
        include( MEERKATINFO_DIRNAME . "/views/view-options.php" );
    }
    ...
    static function instance() {
        ...
        if( !isset( $Meerkat_Info ) ) $Meerkat_Info = new Meerkat_Info();
    }
	...
	function plugin_action_links( $links, $file ) {
		if( $file == plugin_basename( MEERKATINFO_DIRNAME . '/' . basename( __FILE__ ) ) ) {
            $old_links = $links;
            $new_links = array(
     ...
     * form processor.
     * 
     * @uses Meerkat_Info::_admin_options_update()
     */
    function route() {
    ...
    function wp_register_scripts() {
        // Admin JavaScript
        wp_register_script( "{$this->namespace}-admin", MEERKATINFO_URLPATH . "/js/admin.js", array( 'jquery' ), $this->version, true );
    }
    ...
    function wp_register_styles() {
        // Admin Stylesheet
        wp_register_style( "{$this->namespace}-admin", MEERKATINFO_URLPATH . "/css/admin.css", array(), $this->version, 'screen' );
    }
}
if( !isset( $Meerkat_Info ) ) {
	Meerkat_Info::instance();
}

register_activation_hook( __FILE__, array( 'Meerkat_Info', 'activate' ) );
register_deactivation_hook( __FILE__, array( 'Meerkat_Info', 'deactivate' ) );
